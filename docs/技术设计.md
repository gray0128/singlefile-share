...
| 模式 | 技术 | 索引范围 | 触发方式 |
| :--- | :--- | :--- | :--- |
| **语义搜索 (Vector)** | Cloudflare AI + Vectorize | 全文内容 (前 8000 字符) | `type=vector` (默认)；AI/Vectorize 绑定不存在时降级为 Meta |
| **全文精确 (Exact)** | D1 FTS5 | title, description, content | `type=exact` |
| **标题/描述 (Meta)** | SQL LIKE | display_name, description | `type=meta`；或向量绑定缺失时自动降级（零匹配时**不**降级，直接返回空） |
...
- **ID 类型**：Vectorize 返回的 `id` 为字符串，D1 主键为整型，排序时统一通过 `String()` 转换比对，避免类型不匹配导致排序失效
- **零匹配行为**：向量搜索返回空结果时，直接返回 `[]`，**不会**降级为 metadata LIKE 搜索

### 索引建立策略

| 场景 | FTS 索引 | 向量索引 | 说明 |
| :--- | :--- | :--- | :--- |
| **Web 上传** | ✅ 即时 | ✅ Queue 异步 | 主请求立即返回，向量索引由 Consumer 后台处理 |
| **Cron 同步** | ✅ 即时 | ❌ 延后 | 仅读取前 200KB，通过 reindex 补全向量 |
| **管理员 Reindex** | ✅ 补全 | ✅ Queue 异步 | 入队后立即返回，不再逐文件阻塞等待 |
| **Markdown 编辑保存** | ✅ 即时 | ✅ Queue 异步 | 内容更新后自动重建向量索引 |
...
| v1.7 | 2026-02-25 | 修复 AI 语义搜索两处 Bug：① Vectorize 返回字符串 ID 与 D1 整型主键类型不匹配导致排序结果始终为空；② 向量搜索零匹配时静默退化为 metadata LIKE 搜索（现直接返回空列表）；前端新增语义搜索加载状态指示器 |
| v1.8 | 2026-02-25 | 引入 Cloudflare Queues 异步向量索引：上传/编辑/reindex 均改为入队却不预处理，主请求即时返回，Consumer 在后台批量 (max 5 条/批) 调用 Vectorize 建立索引 |
